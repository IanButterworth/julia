...
# The public interface for this is `Pkg.precompile`
function parallel_precompile(;
    ...
    parallel_limiter = Semaphore(num_tasks)

    fancyprint = can_fancyprint(io) && !timing

    direct_deps = [
        PkgId(uuid, name)
        for (name, uuid) in ctx.env.project.deps if !in_sysimage(PkgId(uuid, name))
    ]
    ...
    for dep in ctx.env.manifest
        pkg = PkgId(first(dep), last(dep).name)
        in_sysimage(pkg) && continue
        deps = [PkgId(last(x), first(x)) for x in last(dep).deps]
        depsmap[pkg] = filter!(!in_sysimage, deps)
        # add any extensions
        weakdeps = last(dep).weakdeps
        pkg_exts = Dict{PkgId,Vector{PkgId}}()
        prev_ext = nothing
        for (ext_name, extdep_names) in last(dep).exts
            ...
            for extdep_name in extdep_names
                ...
                if extdep_uuid in keys(ctx.env.manifest.deps) || in_sysimage(PkgId(extdep_uuid, extdep_name))
                    push!(ext_deps, PkgId(extdep_uuid, extdep_name))
                else
    ...
    if ctx_env_pkg !== nothing && isfile(joinpath(dirname(ctx.env.project_file), "src", "$(ctx_env_pkg.name).jl"))
        project_pkgid = PkgId(ctx_env_pkg.uuid, ctx_env_pkg.name)
        depsmap[project_pkgid] = [
            PkgId(last(x), first(x))
            for x in ctx.env.project.deps if !in_sysimage(PkgId(last(x), first(x)))
        ]
        push!(direct_deps, PkgId(ctx_env_pkg.uuid, ctx_env_pkg.name))
    else
    ...
    started = Dict{PkgId,Bool}()
    was_processed = Dict{PkgId,Event}()
    was_recompiled = Dict{PkgId,Bool}()
    for pkgid in keys(depsmap)
        started[pkgid] = false
        was_processed[pkgid] = Event()
    ...
    if !isempty(pkgs)
        ...
        function collect_all_deps(depsmap, dep, alldeps=Set{PkgId}())
    ...
    pkg_queue = PkgId[]
    failed_deps = Dict{PkgId,String}()
    precomperr_deps = PkgId[] # packages that may succeed after a restart (i.e. loaded packages with no cache file)

    print_lock = io isa LibuvStream ? io.lock::ReentrantLock : ReentrantLock()
    first_started = Event()
    printloop_should_exit::Bool = !fancyprint # exit print loop immediately if not fancy printing
    interrupted_or_done = Event()
    ...
    std_outputs = Dict{PkgId,String}()
    taskwaiting = Set{PkgId}()
    pkgspidlocked = Dict{PkgId,String}()
            ...
            while !eof(pipe)
                ...
                if !in(pkg, taskwaiting) && occursin("waiting for IO to finish", str)
                    !fancyprint && lock(print_lock) do
                        println(io, pkg.name, color_string(" Waiting for background task / IO / timer.", warn_color()))
                    end
    ...
    t_print = @async begin
        try
            ...
            i = 1
            last_length = 0
            bar = MiniProgressBar(; indent=2, header="Progress", color=info_color(), percentage=false, always_reprint=true)
                ...
                lock(print_lock) do
                    ...
                    str_ = sprint() do iostr
                        ...
                        if !final_loop
                            ...
                            print(iostr, _truncate_at_width_or_chars(true, str, termwidth), "\n")
                        end
                        for dep in pkg_queue_show
                            loaded = warn_loaded && haskey(loaded_modules, dep)
                            ...
                            line = if dep in precomperr_deps
                                string(color_string("  ? ", warn_color()), name)
                            elseif haskey(failed_deps, dep)
                                string(color_string("  ✗ ", error_color()), name)
                            elseif was_recompiled[dep]
                                ...
                                string(color_string("  ✓ ", loaded ? warn_color() : :green), name)
                            elseif started[dep]
                                ...
                                waiting = if haskey(pkgspidlocked, dep)
                                    ...
                                    color_string(" Being precompiled by $(who_has_lock)", info_color())
                                elseif dep in taskwaiting
                                    color_string(" Waiting for background task / IO / timer. Interrupt to inspect", warn_color())
                                else
                                    ""
                                end
                            ...
                            println(iostr, _truncate_at_width_or_chars(true, line, termwidth))
                        end
                    end
    ...
    for (pkg, deps) in depsmap
        cachepaths = find_all_in_cache_path(pkg)
        sourcepath = locate_package(pkg)
        ...
        elseif project_pkgid isa PkgId
        ...
        task = @async begin
            try
                loaded = haskey(loaded_modules, pkg)
                ...
                is_stale = true
                if !circular && (is_stale = !isprecompiled(pkg; ignore_loaded=true, stale_cache, cachepaths, sourcepath))
                    acquire(parallel_limiter)
                    is_direct_dep = pkg in direct_deps

                    # std monitoring
                    std_pipe = link_pipe!(Pipe(); reader_supports_async=true, writer_supports_async=true)
                    ...
                    try
                        ...
                        t = @elapsed ret = maybe_cachefile_lock(io, print_lock, fancyprint, pkg, pkgspidlocked) do
                            Logging.with_logger(Logging.NullLogger()) do
                                ...
                                compilecache(pkg, sourcepath, std_pipe, std_pipe, false)
                            end
                        end
                        t_str = timing ? string(lpad(round(t * 1e3, digits=1), 9), " ms") : ""
                        if ret isa PrecompilableError
                            push!(precomperr_deps, pkg)
                            !fancyprint && lock(print_lock) do
                                println(io, t_str, color_string("  ? ", warn_color()), name)
                            end
                        else
                            !fancyprint && lock(print_lock) do
                                println(io, t_str, color_string("  ✓ ", loaded ? warn_color() : :green), name)
                            end
                        ...
                        if err isa ErrorException || (err isa ArgumentError && startswith(err.msg, "Invalid header in cache file"))
                            ...
                            !fancyprint && lock(print_lock) do
                                println(io, timing ? " "^9 : "", color_string("  ✗ ", error_color()), name)
                            end
                        else
                            rethrow()
                        end
        ...
        str = sprint() do iostr
            if !quick_exit
                ...
                if n_loaded > 0
                    ...
                    print(iostr, "\n  ",
                        color_string(string(n_loaded), warn_color()),
                ...
                if !isempty(precomperr_deps)
                    ...
                    print(iostr, "\n  ",
                        color_string(string(length(precomperr_deps)), warn_color()),
            ...
            if !isempty(std_outputs)
                ...
                print(iostr, "\n  ", color_string("$(length(std_outputs))", warn_color()), " dependenc$(plural1) had output during precompilation:")
                for (pkgid, err) in std_outputs
                    ...
                    print(iostr, color_string("\n┌ ", warn_color()), name, color_string("\n│  ", warn_color()), err, color_string("\n└  ", warn_color()))
                end
            end
        end
        let str = str
        ...
        if err_str != ""
            ...
            if internal_call # aka. auto-precompilation
                if isinteractive() && !get(ENV, "CI", false)
                    ...
                    println(io, "  ", color_string("$(length(failed_deps))", error_color()), " dependenc$(plural1) errored.")
                    println(io, "  For a report of the errors see `julia> err`. To retry use `pkg> precompile`")
                    setglobal!(MainInclude, :err, PkgPrecompileError(err_msg))
                else
...
function color_string(cstr::String, col::Union{Int64,Symbol})
    enable_ansi = get(text_colors, col, text_colors[:default])
    disable_ansi = get(disable_text_style, col, text_colors[:default])
    return string(enable_ansi, cstr, disable_ansi)
end

function maybe_cachefile_lock(f, io::IO, print_lock::ReentrantLock, fancyprint::Bool, pkg::PkgId, pkgspidlocked::Dict{PkgId,String})
    stale_age = compilecache_pidlock_stale_age
    pidfile = compilecache_pidfile_path(pkg)
    ...
    if cachefile === false
        ...
        !fancyprint && lock(print_lock) do
            println(io, "    ", pkg.name, color_string(" Being precompiled by $(pkgspidlocked[pkg])", info_color()))
        end
        # wait until the lock is available
        FileWatching.mkpidlock(pidfile; stale_age) do
            # double-check in case the other process crashed or the lock expired
            if isprecompiled(pkg; ignore_loaded=true) # don't use caches for this as the env state will have changed
